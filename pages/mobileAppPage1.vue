<template>
  <div>
    <div class="rounded-md shadow-md bg-white block md:hidden">
      ...
      >
        <h2 class="text-xl flex justify-center font-bold mb-2">Inspection AP</h2>

        <h2 class="text-ld font-bold mb-2">Please enter the following details:</h2>
      ...
      <div v-else>
        ...
        <div v-if="questions[currentIndex] && !isEsign">
          <div>
            ...
            <div
              ...
              class="mb-2 px-3"
            >
              <h4 class="text-lg text-blue-900 font-medium mb-2 h-200">Employee Image</h4>
              <div>
            ...
            <div
              ...
              class="mb-2 px-3"
            >
              <h4 class="text-lg text-blue-900 font-medium mb-2 h-200">AP Image</h4>
              <div>
                <div class="w-full border-dotted border-2 rounded h-60 relative my-4">
                  <div
                    class="absolute inset-0 flex flex-col justify-center items-center py-20"
            ...
            <div
              ...
              class="mb-2 px-3"
            >
              <div class="text-gray-600 py-3">Question {{ questions[currentIndex].questionId }} of 31</div>
      ...
      <!-- Add this new section after the questions section -->
    </div>
...
export default {
  ...
  mounted() {
   ...
   let storedSessionId = localStorage.getItem('appSessionId');
  if (!storedSessionId) {
    const now = new Date();
  ...
  methods: {
    async submitSignature() {
      ...
      const esignType = 'ap' // Set the e-sign type (can be "ap" or "emp")
    ...
    async fetchInspectionPDF() {
      try {
        ...
        const response = await axios.get(
          `https://teamap.gwcindia.in/inspection/api/Inspection_pdf.php?sessionId=${sessionId}`,
        )
    ...
    async postData() {
      try {
        ...
        const now = new Date()
        const appSessionId = `${now.getDate().toString().padStart(2, '0')}${(now.getMonth() + 1)
          .toString()
          .padStart(2, '0')}${now.getFullYear().toString().slice(-2)}${now.getHours().toString().padStart(2, '0')}${now
          .getMinutes()
          .toString()
        ...
        if (currentQuestion.questionId === '33') {
          this.fetchInspectionPDF()
        }
